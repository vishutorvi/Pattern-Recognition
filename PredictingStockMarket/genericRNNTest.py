...
trainX=np.array(trainX).reshape(lenX1,1,lenX2)
testX=np.array(testX).reshape(lenX1,1,lenX2)

trainY = np.array(trainY).reshape(lenX1,1,classes)
testY = np.array(testY).reshape(lenX1,1,classes)

model = Sequential()
n1 = 400
n2 = 50
model.add(LSTM(n1, input_shape=(1,lenX2), return_sequences=True, stateful=False))
model.add(LSTM(n2, input_shape=(n1,1), return_sequences=True))
model.add(LSTM(n2, input_shape=(n2,1), return_sequences=True))
#model.add(LSTM(n2, input_shape=(n2,1), return_sequences=True))
#model.add(LSTM(n2, input_shape=(n2,1), return_sequences=True))
model.add(Dense(classes,input_shape=(n2,1)))
...
for a in range(0,len(testY)):
    testindex += [np.argmax(testY[a][-1])]

trainindex = []
for a in range(0,len(trainY)):
    trainindex += [np.argmax(trainY[a][-1])]    
